...
import com.mz.poi.mapper.helper.CellRangeHelper;
import com.mz.poi.mapper.helper.ConstraintGenerator;
...
public class ExcelGenerator {
	...
	public ExcelGenerator(Object excelDto, Workbook workbook) {
		...
		this.constraintGenerator = new ConstraintGenerator(this.workbook);
	}
	...
	private void drawCell(CellStructure cellStructure, Row row, Object rowData, int rowIndex) {
		...
		//밸리데이션
		this.constraintGenerator.createConstraint(cellStructure, cell, annotation.getColumn(), annotation.getCols());
	}
	...
	private void drawArrayCell(CellStructure cellStructure, Row row, Object rowData, int rowIndex) {
		...
		cellDataList.forEach(cellData -> {
			...
			this.bindCellValue(cell, cellGenerator.getCellType(), cellGenerator.getValue(), cellStructure, rowIndex);
			//셀 밸리데이션
			this.constraintGenerator.createConstraint(cellStructure, cell, currentColumn.get(), annotation.getCols());
			//컬럼 증가
	...
	private void mergeCell(Cell cell, int columnIndex, int cols) {
		...
		CellRangeAddress cellRangeAddress = CellRangeHelper.getCellRangeAddress(cell, columnIndex, cols);
		//merge cell
